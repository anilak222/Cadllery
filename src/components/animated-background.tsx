...
export function AnimatedBackground() {
  ...
  return (
    ...
    >
      ...
      {/*
       * Grain overlay is desktop-only. `mix-blend-mode: overlay` forces the
       * compositor to re-read the WebGL canvas every scroll frame, which on
       * iOS Safari shows up as flicker. Hidden on touch devices.
       */}
      <div className="absolute inset-0 hidden opacity-[0.04] mix-blend-overlay pointer-events-none md:block [background-image:url('data:image/svg+xml;utf8,<svg xmlns=%22http://www.w3.org/2000/svg%22 width=%22160%22 height=%22160%22><filter id=%22n%22><feTurbulence type=%22fractalNoise%22 baseFrequency=%220.85%22 numOctaves=%222%22 stitchTiles=%22stitch%22/></filter><rect width=%22100%25%22 height=%22100%25%22 filter=%22url(%23n)%22/></svg>')]" />
    </div>
  );
}
